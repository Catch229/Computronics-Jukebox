...
function createLabel(iText, iXpos, iYpos, iWidth, iColor, iTColor)
	...
	iWidth = iWidth or 1
	local temp = {}
	temp.text = string.sub(iText,1,iWidth)
	temp.xpos = iXpos 
	temp.ypos = iYpos
	temp.width = iWidth
	temp.color = iColor
	...
	function temp.updateTxt(self, iText)
		self.text = string.sub(iText,1,iWidth)
	end
...
function createScrollBox(iX1, iY1, iX2,iY2, iData)
	...
	for i = iY1, iY2, 1 do
		...
		sLbl = createLabel(" ", iX1, i, (iX2-iX1)-2, color, colors.black)
	...
	function temp.updateLabels(self) --Update which data from array is shown on labels. Requires redraw
		for i = 1, #self.labels, 1 do
			self.labels[i].updateTxt(self.labels[i],self.data[i+self.scrollPos-1] or "")
		end
	end
